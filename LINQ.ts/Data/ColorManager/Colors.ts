namespace TypeScript.ColorManager {
    ...
    */
    export function toColorObject(c: string): w3color {
    ...
    export function colorObject(rgb: { r: number, g: number, b: number }, a: number, h: number, s: number) {
        let hsl, hwb, cmyk, ncol, color: w3color, hue, sat;
    ...
    function roundDecimals<T extends IW3color>(c: T): T {
    ...
    function ncsToRgb(ncs: string | RegExpMatchArray) {
        let black, chroma, bc, percent, black1, chroma1, red1, factor1, blue1, red2, green1, green2, blue2, max, factor2, grey;
        let r: number, g: number, b: number;

        ncs = w3trim(<any>ncs).toUpperCase();
        ncs = ncs.replace("(", "");
        ncs = ncs.replace(")", "");
